...
- [ ] Task 1: Set up OpenAI Agents SDK with structured output configuration (AC: 1, 2)
  - [ ] Configure OpenAI API credentials and access in environment variables
  - [ ] Set up vector store IDs for country-specific HS code collections (e.g., "vs_hs_codes_turkmenistan")
  - [ ] Create Pydantic models for structured HS code matching output using `output_type`
  - [ ] Configure Agent with FileSearchTool and structured output format
  - [ ] Test connection to OpenAI Vector Store with sample HS code queries
  - [ ] Document vector store setup and management procedures
